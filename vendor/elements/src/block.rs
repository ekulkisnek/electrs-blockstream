...
impl BlockHeader {
    ...
    pub fn block_hash(&self) -> BlockHash {
        ...
        let mut version = if let ExtData::Dynafed { .. } = self.ext {
            self.version | 0x8000_0000
        } else {
            self.version
        };
        if self.withdrawal_bundle_hash.is_some() {
            version |= 0x4000_0000;
        }
...
impl Encodable for BlockHeader {
    fn consensus_encode<S: io::Write>(&self, mut s: S) -> Result<usize, encode::Error> {
        ...
        let mut len = version.consensus_encode(&mut s)? +
...
impl Decodable for BlockHeader {
    fn consensus_decode<D: io::Read>(mut d: D) -> Result<Self, encode::Error> {
        ...
        let has_withdrawal_bundle = version & 0x4000_0000 != 0;
        version &= 0xbfff_ffff;

        let prev_blockhash = Decodable::consensus_decode(&mut d)?;
        let merkle_root = Decodable::consensus_decode(&mut d)?;
        let withdrawal_bundle_hash = if has_withdrawal_bundle {
...
mod tests {
    ...
    fn withdrawal_bundle_header_roundtrip() {
        let mut block: Block = hex_deserialize!(&SIMPLE_BLOCK);
        let original_hash = block.block_hash();
        let bundle_hash = BlockHash::from_slice(&[0x5a; 32]).unwrap();
        block.header.withdrawal_bundle_hash = Some(bundle_hash);

        let encoded = serialize(&block);
        assert_eq!(&encoded[..4], &[0x00, 0x00, 0x00, 0x60]);
        assert_ne!(block.block_hash(), original_hash);

        let decoded: Block = encode::deserialize(&encoded).unwrap();
        assert_eq!(decoded.header.version, 0x2000_0000);
        assert_eq!(decoded.header.withdrawal_bundle_hash, Some(bundle_hash));
